...
export default function PlaybackPage() {
  const { voiceLines, setVoiceLines } = useVoiceLines();
  ...
  const playVoiceLine = (index: number) => {
    ...
    newAudio.addEventListener("timeupdate", () => {
      ...
      setOverallTime(
        individualDurations
          .slice(0, index)
          .reduce((sum, duration) => sum + duration, 0) + newAudio.currentTime
      );
    });
  ...
  // Add this useEffect to handle persistence
  useEffect(() => {
    // Store voice lines in localStorage whenever they change
    if (voiceLines.length > 0) {
      localStorage.setItem("voiceLines", JSON.stringify(voiceLines));
    }
  }, [voiceLines]);

  // Add this useEffect to load persisted voice lines on mount
  useEffect(() => {
    const storedVoiceLines = localStorage.getItem("voiceLines");
    if (storedVoiceLines && voiceLines.length === 0) {
      // You'll need to add a setVoiceLines function to your context
      setVoiceLines(JSON.parse(storedVoiceLines));
    }
  }, []);
    ...
    >
        ...
        <div className="container mx-auto flex flex-col gap-2">
          ...
          <div className="w-full bg-white/20 h-1 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${(getCurrentOverallTime() / totalDuration) * 100}%`,
              }}
            />
          </div>
      ...
      <div className="container mx-auto p-6 flex-1 flex items-center">
        <div className="space-y-6 w-full max-w-2xl mx-auto">
          {voiceLines.map((line, index) => (
            ...
            >
              <p className="text-sm mb-1 opacity-80">{line.role}</p>
      ...
      {voiceLines.length === 0 && (
        <div className="flex-1 flex items-center justify-center">
          <p className="text-center opacity-50">No voice lines available</p>
        </div>
      )}
    </div>
  );
